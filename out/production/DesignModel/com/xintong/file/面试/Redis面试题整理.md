...
主从：主机写从机读，缓解压力，但是主挂掉的话得手动却切换

哨兵：可以用哨兵集群对主从进行监控，实现故障自动转移，但是同样，写还是往主机写，写压力大的时候不适用

集群：0-16383 集群可以分散写压力
...
3. _缓存雪崩_：
   ...
   事中，一旦缓存异常，立马开启限流并降级，保证数据库不被打死
   
   事后，万一数据库被打死，先重启缓存，保证rdb和aof都要有，aof开启重写，并且开启混合持久化，以保证数据完整性和回复速度，再重启数据库
